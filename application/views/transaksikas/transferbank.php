		...
		<div class="panel-content">
			<div class="row">
				<div class="col-sm-9 col-lg-9">
					<h4 class="color-primary">Data Transaksi Transfer Antar Bank</h4>
				</div>
				<div class="col-sm-3  col-lg-3 ">
					<a href="<?php echo base_url(); ?>kas-transfer-add-bank" class="btn btn-primary btn-block">
						<i class="fa fa-plus"></i>
						Tambah Transfer Antar Bank
					</a>
				</div>
			</div>
		</div>
				...
				<tbody>
					<?php if ($data_source->num_rows() > 0) {
						...
						foreach ($data_source->result() as $key) {
							...
							$this->db->where('IDAKUN',$key->DARI_BANK); 
							$getuntukbank = $this->db->get()->row();
							$DARI_BANK = $getuntukbank->JENIS_TRANSAKSI; 
							
							
							$this->db->select('*');
							$this->db->from('jns_akun');
							$this->db->where('IDAKUN',$key->UNTUK_BANK); 
							$getuntukbank = $this->db->get()->row();
							$UNTUK_BANK = $getuntukbank->JENIS_TRANSAKSI; 
					...
					?>
							<tr>
								...
								<td><?php echo $DARI_BANK ?></td>
								<td><?php echo $UNTUK_BANK ?></td>
