...
class Pdftc extends TCPDF {
	...
	public function set_nsi_headers($nsi_header,$norek, $nama, $alamat,$kodecabang) {
        if($nsi_header == TRUE) {
			$this->CI =& get_instance();
			$this->CI->load->database();
			
            $this->CI->db->select('*');
            $this->CI->db->from('m_cabang');
            $this->CI->db->where('KODE', $kodecabang);
			$getcabang = $this->CI->db->get()->row();
            ...
            $nsi_header = '<table style="width:100%;" cellspacing="0" cellpadding="2">
                    <tr>
						...
						<td style="width:255px; font-weight:bold; text-align:center; "> 
							<strong style="font-size: 14px;"> '.$getcabang->NAMA.'</strong> <br><br>  
							<table style="width:250px; font-weight:bold; text-align:center; "> 
                                <tr>
                                    <td>'.$getcabang->ALAMAT.'</td>
                                </tr>
                                <tr>
                                    <td>'.$getcabang->TELP.'</td>
                                </tr>
                            </table>
						</td>
                     </tr>
	...
	public function set_nsi_headerss($nsi_header,$norek, $nama, $periode, $tertanggal,$kodecabang) {
        if($nsi_header == TRUE) {
			$this->CI =& get_instance();
			$this->CI->load->database();
