...
class Struk extends TCPDF {
    ...
    public function set_nsi_header($nsi_header) {
        if($nsi_header == TRUE) {
            $this->CI->load->model('ModelSimpanan');
            $opsi_val_arr = $this->CI->ModelSimpanan->getCabang();
            ...
            $nsi_header = '<table style="width:50%;">
                    <tr>
                        <td style="width:10%;"><img src="assets/theme_admin/img/logo.png" width="70" height="70"></td>
                         <td style="width:90%;"><strong style="font-size: 14px;"> '.$opsi_val_arr->NAMA.'</strong> <br>
                            <table>
                                <tr><td>'.$opsi_val_arr->ALAMAT.' Tel.'.$opsi_val_arr->TELP.'</td></tr>
                                <tr><td>Email :'.$opsi_val_arr->EMAIL.'</td></tr>
                                <tr><td>Web :'.$opsi_val_arr->WEB.'</td></tr>
                            </table>
                         </td>
                     </tr>
                </table>
