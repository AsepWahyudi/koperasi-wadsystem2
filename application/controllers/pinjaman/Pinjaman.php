...
class Pinjaman extends CI_Controller {
	...
	public function save(){
		...
		if($this->dbasemodel->insertData('tbl_pinjaman_h', $save)) {
			...
			$sqlpinjam   = "SELECT IDPINJM_H FROM tbl_pinjaman_h ORDER BY IDPINJM_H DESC LIMIT 1";
			$getidpinjam = $this->dbasemodel->loadsql($sqlpinjam)->row();
			$IDPINJM_H   = $getidpinjam->IDPINJM_H;
			
			$insertanggunan = array(
								'IDPINJM_H'  => $IDPINJM_H,
								'DATE_INPUT' => date("Y-m-d H:i:s") 
							); 
			$this->dbasemodel->insertData('m_anggunan', $insertanggunan);
