...
class Pinjaman extends CI_Controller {
    ...
    public function formadd(){
		...
		$sql	=	sprintf("SELECT IDJAMINAN, NAMAJAMINAN
							 FROM
							 	jns_jaminan 
							 ORDER BY NAMAJAMINAN ASC");
	...
	public function save(){
		...
		$basil_dasar = $basil_total / $lama_angsuran;
	...
	public function detail(){
		...
		$sql	=	sprintf("SELECT
								IDPINJ_D, TGL_BAYAR, ANGSURAN_KE, BAYAR_SALDO, BASILBAYAR, POKOKBAYAR,
								JUMLAH_BAYAR, DENDA_RP, TERLAMBAT, KET_BAYAR, DK, KAS_ID, 
								JENIS_TRANS, UPDATE_DATA, USERNAME, KETERANGAN
							 FROM 
							 	tbl_pinjaman_d A
							 WHERE 
							 	A.IDPINJAM = %s ", $id );
		...
		$sql	=	sprintf("SELECT
								ID, TANGGAL, JUMLAH, ANGSURAN_KE, LUNAS, JENIS
							 FROM 
							 	tbl_reset A
							 WHERE 
	...
	public function save_angsuran(){
		...
		$ceklst			=	$this->dbasemodel->loadsql("Select * FROM checklist_teller WHERE TGL_AWAL='".date("Y-m-d")."' 
										...
										AND KODECABANG='".$this->session->userdata('wad_kodecabang')."'"); //AND Jenis='Angsuran'
