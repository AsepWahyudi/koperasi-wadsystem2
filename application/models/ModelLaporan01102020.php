...
class ModelLaporan extends CI_Model
{
	...
	public function getDataTableAktif($keyword, $dataperpage, $page)
	{
		...
		$basequery = sprintf("SELECT FILE_PIC, IDANGGOTA, NOREK, NAMA, '' AS NAMABANK, JK,
						      CONCAT_WS('', KODEPUSAT, '.', KODECABANG, '.', NO_ANGGOTA, '') KODE_ANGGOTA,
	...
	public function getDataKasAnggota($keyword, $dataperpage, $page)
	{
		...
		$datastart    = $startid + 1;
		$dataend      = $datastart + $dataperpage - 1;
		// $con_keyword .= $this->cabang != "" ? " AND A.KODECABANG = '" . $this->cabang . "' " : "";
		/* if($this->session->userdata("wad_level") == "admin")
		{
			$con_keyword .= "";
		}
		...
		if($this->session->userdata("wad_level") == "admin")
		{
			$basequery = sprintf("SELECT A.IDANGGOTA, A.FILE_PIC, A.NAMA, JK, A.ALAMAT, A.TELP, A.JABATAN,
								  CONCAT_WS('', A.KODEPUSAT, '.', A.KODECABANG, '.', A.NO_ANGGOTA, '') KODE_ANGGOTA,
	...
	public function getBukubesar($keyword, $dataperpage, $page, $post)
	{
		...
		$datastart    = $startid + 1;
		$dataend      = $datastart + $dataperpage - 1;
		// $con_keyword .= $this->cabang != "" ? " AND B.KODECABANG = '" . $this->cabang . "' " : "";
		// if($this->session->userdata("wad_level") == "admin")
		// {
			// $con_keyword .= "";
		// }
		...
		$tgl          = explode('-', $post['tgl']);
		$con_keyword .= !empty($post['tgl']) ? " AND DATE(B.TANGGAL) BETWEEN '" . date('Y-m-d', strtotime(str_replace('/', '-', trim($tgl[0])))) . "' AND '" . date('Y-m-d', strtotime(str_replace('/', '-', trim($tgl[1])))) . "'" : "";
	...
	public function getJurnal($keyword, $dataperpage, $page, $post)
	{
		...
		$datastart    = $startid + 1;
		$dataend      = $datastart + $dataperpage - 1;
		// $con_keyword  = $this->cabang != "" ? " B.KODECABANG = '" . $this->cabang . "' " : " 1=1 ";
		// if($this->session->userdata("wad_level") == "admin")
		// {
			// $con_keyword = "";
		// }
