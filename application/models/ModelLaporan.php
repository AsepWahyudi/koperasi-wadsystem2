...
class ModelLaporan extends CI_Model
{
	...
	public function getNeraca($keyword, $dataperpage, $page, $post)
	{
		...
		{
			if($post['plhcabang'] != "" AND $post['plhcabang'] != "4")
			{
				
				$cabs = $this->db->query("SELECT KODEAKUN AS STRLIKE FROM m_cabang WHERE KODE NOT IN('4','".$post['plhcabang']."')")->result();
				
				/* $this->db->select('KODEAKUN');
				$this->db->from('m_cabang');
				$this->db->where('KODE',$post['plhcabang']); 
				$output = $this->db->get()->row(); 
				
				$this->db->select('IDAKUN');
				$this->db->from('jns_akun');
				$this->db->where('KODE_AKTIVA',$output->KODEAKUN); 
				$outputs = $this->db->get()->row();  */
				
				$con_keyword .="  AND C.KODECABANG = '" .$post['plhcabang']. "'";
				
				$con_akun =' ';
				foreach($cabs as $rows){
					
					$con_akun .= " AND A.KODE_AKTIVA NOT LIKE '".$rows->STRLIKE ."%' ";
				}
				
				$con_akun .="AND A.PARENT NOT IN('4') AND A.IDAKUN !='4' ";
				// $con_akun .= " AND A.IDAKUN IN('1','2','3','".$outputs->IDAKUN ."') OR A.PARENT = '". $outputs->IDAKUN ."'";
				// $con_akun .= " AND A.PARENT IN('0','1','2','".$outputs->IDAKUN ."') OR A.IDAKUN = '". $outputs->IDAKUN ."'";
				// $con_akun .=	" OR A.IDAKUN = '". $outputs->IDAKUN ."'"; 
			}
			if($post['plhcabang'] != "" AND $post['plhcabang'] == "4")
			{
				
				$cabs = $this->db->query("SELECT KODE, KODEAKUN AS STRLIKE FROM m_cabang WHERE KODE NOT IN('".$post['plhcabang']."')")->result();
			    ...
			    $con_akun .=" AND A.IDAKUN NOT IN('7','11','15','35','19','20','23','27','31','39','435','437','449') ";  
			 
			}
			 
		}
		   
		$basequery = "SELECT A.IDAKUN, A.PARENT, A.KODE_AKTIVA, A.JENIS_TRANSAKSI, A.AKUN,
						(
							SELECT
						...
						FROM jns_akun A
						WHERE 
						A.AKUN IN ('Aktiva', 'Pasiva', 'Equity') $con_akun";
	...
	public function getLabaRugi($keyword, $dataperpage, $page, $post)
	{
		...
		$basequery = "SELECT 
						...
						jns_akun A        
						WHERE
						(A.AKUN LIKE 'pendapatan' OR A.AKUN LIKE 'beban') AND A.AKTIF = 'Y' $con_akun";
