...
{ 
	...
	public function lap_data_simpanan($id){
		$this->db->select('*');
	...
	public function lap_data_simpananrow($id){
		$this->db->select('*');
		$this->db->from('transaksi_simp');
		$this->db->where('ID_TRX_SIMP',$id);
		$query = $this->db->get();
	...
	public function getCabang(){
		...
		$this->db->where('KODE',$this->session->userdata('wad_kodecabang'));
		$query = $this->db->get()->row();
		
		return $query;
	}
	public function getCabangh($id){
		$this->db->select('*');
		$this->db->from('m_cabang');
	...
	public function get_key_val(){
		$out = array();
		$this->db->select('idSB,OPSI_KEY,OPSI_VAL');
	...
	public function get_data_anggota($id){
		$this->db->select('m_anggota.*, m_cabang.NAMA as NAMACABANG');
	...
	public function get_jenis_simpan($id){
		$this->db->select('*');
	...
	public function getDataTable($keyword, $dataperpage, $page, $koncabang, $wheretrgl){
		...
		$sql = sprintf("SELECT A.ID_TRX_SIMP, DATE_FORMAT(A.TGL_TRX, '%s') TGL_TRX, A.NAMA_PENYETOR, A.ALAMAT, FORMAT(A.JUMLAH, 0) JUMLAH, A.USERNAME, A.NO_IDENTITAS, B.JNS_SIMP,D.KODECABANG,E.NAMA AS NAMACABANG,A.KETERANGAN FROM transaksi_simp A LEFT JOIN jns_simpan B ON A.ID_JENIS = B.IDAKUN LEFT JOIN m_user D ON A.USERNAME = D.USERNAME LEFT JOIN m_cabang E ON D.KODECABANG = E.KODE WHERE STATUS = 1 AND UPDATE_DATA <> '0000-00-00 00:00:00' AND DK = 'D' AND %s %s %s ORDER BY DATE(A.TGL_TRX) DESC, A.ID_TRX_SIMP DESC", '%d/%m/%Y', $con_keyword, $koncabang, $wheretrgl);
	...
	public function updateSaldo($tipe = 'tambah', $idtrx){
		...
		$sql = sprintf( "SELECT A.ID_ANGGOTA, A.JUMLAH, A.ID_JENIS FROM transaksi_simp A WHERE ID_TRX_SIMP = %s", $idtrx );
	...
	public function getSaldo($idanggota, $idjenis){
		...
		$sql = sprintf("SELECT SALDO FROM m_anggota_simp WHERE IDANGGOTA = %s AND IDJENIS_SIMP IN (%s) ", $idanggota, $idjenis);
	...
	public function getSaldo2222($idanggota, $idjenis){ 
		...
		$sql   = sprintf("SELECT %s AS SALDO FROM m_anggota WHERE IDANGGOTA = %s", $field, $idanggota); 
		$query = $this->db->query($sql); 
		$row   = $query->row();
